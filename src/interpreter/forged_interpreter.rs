use sparksrs::Rolls;
use crate::interpreter::{Reply, RollStatus::*, ForgedType, ForgedType::*};
use std::fmt::Write as _;

fn forged_dice(rolls: Rolls, roll_type: ForgedType, zero_d: bool) -> Reply {
    let sixes = rolls.dice.iter().filter(|&die_value| *die_value == 6).count();
    ...
    let title = match roll_type {
        ...
        Resist => {
            if status == Crit { String::from("Critical!") } else {
        ...
        Clear => { format!("Clear **{}** stress.", score) }
    };
    ...
    if sixes > 1 {
        ...
        match roll_type {
            ...
            Downtime => write!(description, " **{} ticks** on the relevant clock.", match status {
                Crit => 5,
                FullSuccess => 3,
                MixedSuccess => 2,
                Failure => 1
